...
import { loadPapers, getPaperById } from './data.js';
...
function renderPaperCard(paper, modes = []) {
    const summaryPreview = paper.summary.researchQuestions
    ...
    <div class="card" data-paper-id="${paper.id}" tabindex="0" role="button" aria-label="View ${escapeHtml(paper.title)}">
      ...
      <p class="card-summary">${escapeHtml(summaryPreview)}</p>
      <div class="card-tags">
...
export async function renderPaperDetail(container, paperId) {
    ...
    const sections = [
        { key: 'researchQuestions', label: 'Research Questions', icon: '❓' },
    ...
    for (const section of sections) {
        const content = paper.summary[section.key];
        if (content) {
            html += `
